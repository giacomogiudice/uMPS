...
%
% This rubroutine is called when H is a cell array of operators, each
% acting on the site of a unit cell. The behavior should be typically the
% same, except that it will return a cell array containing the tensors
% associated to a single unit cell.
...
%
%
% [A_left,A_right,C,A,output,blocks,stats] =
% vumps_multicell(H,D_list,d,settings) H must be specified as a (Nx1) cell
% array. The resulting A_left, A_right, C, A are all cell-arrays of the
% same dimension.
%
% See also: VUMPS.
N = length(H);
