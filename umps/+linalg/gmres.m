function varargout = gmres(A,B,options)
% BICGSTAB Wrapper for gmres supporting tensors.
%
% x = gmres(A,b)
...
%
% See also: GMRES, ALGORITHMS/VUMPS.
if isnumeric(A)
    Afun = A;
    b = B;
else
...
if exist('options','var') && ~isempty(options) && ~isempty(fieldnames(options))
    ...
    [varargout{1:nargout}] = gmres(Afun,b,n,options.tol,options.maxit,[],[],options.v0);
else
    [varargout{1:nargout}] = gmres(Afun,b,n);
end
